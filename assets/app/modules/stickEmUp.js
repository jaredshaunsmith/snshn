...
snshn.StickEmUp = (function($, snshn) {
    ...
    var def = function(el) {
        ...
        this.root = this.$els.eq(0).offset().top;
            ...
            this.$els.each(function(i) {
                ...
                if(pos <= $(window).scrollTop()) {
                    ...
                    if(next.length > 0 && stick.offset().top >= $.data(stick[0], 'pos') - stick.outerHeight()) {
                        stick.addClass(self.states.abs).css('top', $.data(next[0], 'pos') - (stick.outerHeight() * 2) - (self.root - self.$els.eq(i).outerHeight()) );
                    }
                } else {
